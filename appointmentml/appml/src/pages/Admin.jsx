...
export default function Admin() {
    ...
    const confirmDeleteContact = async () => {
        ...
        try {
            await adminApi.deleteContact(pendingDeleteContactId)

            setContacts((previous) =>
                previous.filter(
                    (contact) =>
                        contact._id !== pendingDeleteContactId
                )
            )
    ...
    const filteredContacts = useMemo(() => {
        ...
        if (contactFilter === 'unread') {
            return sorted.filter((contact) => !contact.read)
        }
                                ...
                                {filteredContacts.length === 0 ? (
                                    <div className='bg-white rounded-3xl p-16 text-center shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-purple-50'>
                                        ...
                                        <p className='text-slate-400 text-sm mt-1'>
                                            New messages submitted from the Contact Us page will appear here.
                                        </p>
                                    </div>
                                ) : (
                                            ...
                                            >
                                                <div className='flex items-start justify-between gap-4'>
                                                    <div className='flex items-start gap-4 min-w-0'>
                                                        <div className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 ${contact.read
                                                    ...
                                                    <button
                                                        type='button'
                                                        onClick={() =>
                                                            setPendingDeleteContactId(contact._id)
                                                        }
                                                ...
                                                <div className='mt-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3'>
                                                    ...
                                                    {!contact.read && (
                                                        <button
                                                            ...
                                                            onClick={() =>
                                                                handleMarkContactRead(contact._id)
                                                            }
